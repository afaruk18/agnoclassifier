...
class ScoreItemWithRationale(BaseModel):
    ...
    rationale: str = Field(..., description="Brief explanation for why this specific score was assigned to this class based on the document content.")


class LegalClassificationOutput(BaseModel):
    best_class_title: str = Field(..., description="Title of the best matching legal class.")
    rationale: str = Field(..., description="Brief explanation for why this specific score was assigned to this class based on the document content. For low scores, What did you catch?")
    key_terms: List[str] = Field(..., description="Key legal terms found in the document that support the classification.")
    scores: List[ScoreItemWithRationale] = Field(..., description="List of scores and rationales for all legal classes.")
...
class LegalDocumentClassifier:
    ...
    def __init__(self, 
                ...
                legal_classes: Optional[List[Dict[str, str]]] = None,
        ...
        self.class_list_str = "\n".join([f"- ID: {lc['id']}, Title (TR): {lc['title_tr']}" for lc in self.legal_classes])
    ...
    def _create_agent(self, document_text: str) -> Agent:
        ...
        if len(document_text) > self.max_chars_for_prompt:
            print(f"Warning: Document text truncated to {self.max_chars_for_prompt} characters for the prompt.")
    ...
    def classify_text(self, document_text: str, source_file: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            results = {
                ...
                "raw_scores": [(score.title, score.score, score.rationale) for score in response.content.scores],
